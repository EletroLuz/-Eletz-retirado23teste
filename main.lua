...
local function cleanup_after_helltide()
    ...
    -- Clear the actors blacklist
    --if actors and actors.clear_blacklist then
        --actors.clear_blacklist()
    --end

    -- Reset automindcage variables
    --if automindcage and automindcage.reset then
        --automindcage.reset()
    --end

    -- Reset revive module
    --if revive and revive.reset then
        --revive.reset()
    --end
    ...
    if plugin_enabled then
        if teleporting then
            ...
            if current_world:get_name():find("Limbo") then
                ...
                if loading_start_time and (current_time - loading_start_time) < 15 then --was 4 add 15 secs
                    return
                end
                loading_start_time = nil
            end
            ...
            if not is_loading_screen() then
                ...
                if world_instance then
                    ...
                    elseif os.clock() > next_teleport_attempt_time then
                        console.print("Retrying, teleport...") -- era Teleport failed, retrying...
            ...
            if local_player then
                ...
                if current_in_helltide then
                    ...
                    console.print("Not in Helltide zone. Attempting to teleport in 5 seconds.")
                    current_city_index = (current_city_index % #helltide_tps) + 1
                    -- Add a delay of 5 seconds
                    local delay_start = os.clock()
                    while os.clock() - delay_start < 5 do
                    -- You might want to add a small sleep here to prevent busy waiting
                    -- If available in your environment, you could use something like:
